...
import { FaChevronLeft, FaChevronRight, FaPlay, FaPause, FaCog, FaTasks } from 'react-icons/fa'; // アイコン用

// スライド1枚分の型（interface）
interface Slide {
    icon: React.ReactElement; // アイコン（React要素）
    title: string;            // タイトル
...
const slides: Slide[] = [
    ...
    // さらにスライドを追加したいときはここに書く！
];

// UsageCarouselコンポーネント
export const UsageCarousel = () => {
    ...
    const goToPrevious = () => {
        ...
        const newIndex = isFirstSlide ? slides.length - 1 : currentIndex - 1; // 先頭だったら最後に戻る
    ...
    const goToNext = () => {
        ...
        const newIndex = isLastSlide ? 0 : currentIndex + 1; // 最後だったら先頭に戻る
        setCurrentIndex(newIndex);
    };

    // 特定のスライドにジャンプする（ドットボタン用）
        ...
        <div className="card bg-base-100 shadow-xl mt-8 p-6 relative">
            ...
            <div className="overflow-hidden">
                <div
                    ...
                    style={{ transform: `translateX(-${currentIndex * 100}%)` }} // インデックスに応じて左右にスライドする
                >
